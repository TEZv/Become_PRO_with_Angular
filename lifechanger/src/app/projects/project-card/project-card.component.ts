...
export class ProjectCardComponent {
  ...
  private svgTemplateLink: string = `
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none">
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes['isDarkMode']) {
      this.updateSVGSource();
      console.log('Updated SVG Source:', this.githubSvgSource, this.linkSvgSource);
    }
  }
  ...
  updateSVGSource(): void {
    const githubFillColor = this.isDarkMode ? '#fff' : '#871781';
    ...
    const linkFillColor = this.isDarkMode ? '#3fb65bcb' : '#871781';
    const linkSvgContent = this.svgTemplateLink.replace(
      'FILL_COLOR_LINK',
      linkFillColor
    );
    this.linkSvgSource =
      'data:image/svg+xml;charset=UTF-8;base64,' + btoa(linkSvgContent);
  }
}
